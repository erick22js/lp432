...
cpuInterr _cpuPush8(Cpu *cpu_s, uint8 data){
	cpu_s->gregs[GREG_ESP]--;
	uint32 adr = cpu_s->gregs[GREG_ESP];
...
cpuInterr _cpuPush16(Cpu *cpu_s, uint16 data){
	cpu_s->gregs[GREG_ESP] -= 2;
	uint32 adr = cpu_s->gregs[GREG_ESP];
	cpuTrAdrWriteStack(adr);
	cpuWriteBus16(adr, data);
...
cpuInterr _cpuPush32(Cpu *cpu_s, uint32 data){
	cpu_s->gregs[GREG_ESP] -= 4;
	uint32 adr = cpu_s->gregs[GREG_ESP];
	cpuTrAdrWriteStack(adr);
	cpuWriteBus32(adr, data);
...
// Memory Code Access
cpuInterr _cpuFetch8(Cpu *cpu_s){
...
cpuInterr _cpuFetch16(Cpu *cpu_s){
	uint32 adr = cpu_s->reg_pc;
	cpu_s->reg_pc += 2;
	cpuTrAdrCode(adr);
...
cpuInterr _cpuFetch32(Cpu *cpu_s){
	uint32 adr = cpu_s->reg_pc;
	cpu_s->reg_pc += 4;
	cpuTrAdrCode(adr);
...
cpuInterr _cpuFetchOpcode(Cpu *cpu_s){
	cpuFetch8();
...
cpuInterr _cpuFetchOS(Cpu *cpu_s){
	cpuFetch8();
	uint8 os = cpu_s->code;
...
cpuInterr _cpuFetchMemIndex(Cpu *cpu_s){
	cpuFetch8();
	uint8 ns = cpu_s->code;
	uint8 regi = ns&0xF;
	uint8 adrm = (ns>>4)&0xF;
...
cpuInterr _cpuFetchMV8(Cpu *cpu_s){
	cpuFetch8();
	cpu_s->mv = cpu_s->code;
	return 0;
}

cpuInterr _cpuFetchMV16(Cpu *cpu_s){
	cpuFetch16();
	cpu_s->mv = cpu_s->code;
	return 0;
}

cpuInterr _cpuFetchMV32(Cpu *cpu_s){
	cpuFetch32();
	cpu_s->mv = cpu_s->code;
	return 0;
}
