...
bool procTestOperand(uint8 cond, uint32 opr){
	switch (cond){
		case 0x0: { // Equal Zero
			return opr==0;
		};
...
cpuInterr proc21(){
	...
	cpuFetchOS();
	cpuFetch16();
	if (procTestFlags(cpu_s.os_desc)){
		cpuJumpBy(u16ToInt(cpu_s.code));
	}
	return 0;
}
...
cpuInterr proc22(){
	...
	cpuFetchOS();
	cpuFetch16();
	if (procTestOperand(cpu_s.os_desc, cpuReadReg32(cpu_s.os_regm))){
		cpuJumpBy(u16ToInt(cpu_s.code));
	}
	return 0;
}
